...
solution_matrix = curve_matrix @ independent_matrix

fig, ax = plt.subplots()

lins = np.linspace(np.min(x), np.max(x), 1000)
fit = np.poly1d(solution_matrix[::-1])

ax.errorbar(*data, capsize=4, color="tab:orange", fmt="o", ls=None)
ax.plot(lins, fit(lins), color="tab:purple", ls="solid")
...
std_dev = np.sqrt(N*chisq / ((N-fit_order-1) * np.sum(sy**-2)))
...
for i, (fit_parameter, fit_unc) in enumerate(zip(solution_matrix, solution_unc)):
